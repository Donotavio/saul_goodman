...
async function hydrate() {
    try {
        const response = await sendRuntimeMessage('metrics-request');
        if (!response?.metrics) {
...
function calculateMaxMinutes(buckets) {
...
function renderCompositionChart(metrics) {
...
function renderStoryList(metrics, kpis) {
    ...
    const items = [
        ...
        topProcrastination
            ? {
                title: 'Vilão do dia',
                body: `${topProcrastination.domain} drenou ${formatDuration(topProcrastination.milliseconds)}.`
            }
            : null,
        longestIdle
            ? {
                title: 'Maior silêncio',
                body: `Entre ${formatTimeRange(longestIdle.startTime, longestIdle.endTime, locale)} o navegador ficou parado por ${formatDuration(longestIdle.durationMs)}.`
            }
            : null
    ].filter(Boolean);
    if (!items.length) {
...
function renderTimeline(entries) {
    ...
    meaningful.forEach((entry) => {
        ...
        li.appendChild(span);
        li.appendChild(document.createTextNode(`${entry.domain} • ${formatDuration(entry.durationMs)} • ${describeCategory(entry.category)}`));
        timelineListEl.appendChild(li);
    });
}
...
function findLongestSegment(timeline, category) {
