...
function buildDurations(heartbeats, options = {}) {
  ...
  for (let i = 0; i < ordered.length; i++) {
    ...
    if (!hasNext || delta === null || delta > gapMs) {
      // BUG-FIX: Prevent accumulating future time by clamping endTime to now
      const now = Date.now();
      
      if (active.heartbeatCount > 1) {
        const projectedEnd = heartbeat.time + Math.min(graceMs, gapMs);
        const end = Math.min(projectedEnd, now); // Clip to current time
        active.endTime = Math.max(active.endTime, end);
      } else {
        const minDuration = Math.min(30 * 1000, graceMs);
        const projectedEnd = heartbeat.time + minDuration;
        const end = Math.min(projectedEnd, now); // Clip to current time
        active.endTime = Math.max(active.endTime, end);
      }
      pushActive();
    ...
    if (delta > 0) {
      // BUG-FIX: Clip to now to prevent future time
      const now = Date.now();
      const clippedNextTime = Math.min(next.time, now);
      active.endTime = Math.max(active.endTime, clippedNextTime);
    }
  }
